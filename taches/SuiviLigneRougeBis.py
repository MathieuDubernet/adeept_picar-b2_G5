...
class SuiviLigneRouge:
    ...
    def __init__(self):
        ...
        # Masque bleu HSV : marqueur d'arrivee.
        self.BLEU_BAS = np.array([100, 100, 70]); self.BLEU_HAUT = np.array([130, 255, 255])

        self.MIN_PIXELS_BLEU        = 400    # presence de bleu dans l'image entiere
        self.DELAI_ARRET_APRES_BLEU = 0.5    # s sans bleu visible avant l'arret
    ...
    def pixels_bleus(self, bgr):
        hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
        masque = cv2.inRange(hsv, self.BLEU_BAS, self.BLEU_HAUT)
    ...
    def main(self):
        """
        Boucle principale du suivi de ligne rouge avec arrivee sur carre bleu.
        ...
        Quand le carré bleu est aperçu, le robot continue son suivi et ne
        s'arrête que lorsque le bleu a quitté le champ par le bas depuis 0.5s
        (carré franchi). En secours, arrêt si la bande basse reste vide trop
        ...
        try:
            ...
            while True:
                ...
                n_bleu, y_bleu = self.pixels_bleus(bgr)
                if n_bleu > self.MIN_PIXELS_BLEU:
                    if not bleu_vu:
                        print(f"\n[ARRIVEE] Carre bleu apercu ({n_bleu} px) - "
                              f"poursuite jusqu'a disparition par le bas.")
                    bleu_vu = True
                    t_dernier_bleu = t
                    y_dernier_bleu = y_bleu
                elif (bleu_vu
                      and (t - t_dernier_bleu) >= self.DELAI_ARRET_APRES_BLEU
                      and y_dernier_bleu >= self.SORTIE_BAS_Y):
                    if self.MOTEUR_ACTIF:
                        moteur.motorStop()
                    print(f"\n[ARRIVEE] Bleu sorti par le bas (y={y_dernier_bleu}) "
                          f"et hors champ depuis {t - t_dernier_bleu:.1f}s - arret.")
                    break
